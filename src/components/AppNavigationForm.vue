...
export default {
		...
		clipboardTooltip() {
			if (this.copied) {
				return this.copySuccess
					? t('build', 'Form link copied')
					: t('build', 'Cannot copy, please copy the link manually')
			}
			return t('build', 'Copy share link')
		},
	},
	...
	methods: {
		...
		async onDeleteForm() {
			if (!confirm(t('build', 'Are you sure you want to delete {title}?', { title: this.formTitle }))) {
				return
			}
			...
			try {
				...
				showError(t('build', 'Error while deleting {title}', { title: this.formTitle }))
