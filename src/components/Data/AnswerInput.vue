<template>
	<li class="question__item">
		...
		<input
			ref="input"
			:aria-label="t('build', 'An answer for the {index} option', { index: index + 1 })"
			:placeholder="t('build', 'Answer number {index}', { index: index + 1 })"
			:value="answer.text"
	...
	methods: {
		...
		async createAnswer(answer) {
			try {
				...
				showError(t('build', 'Error while saving the answer'))
				console.error(error)
			}
