	...
	<AppContent v-else>
		<TopBar>
			<button @click="showEdit">
				...
				{{ t('build', 'Back to questions') }}
			</button>
		</TopBar>

		<header v-if="!noSubmissions">
			<h2>{{ formTitle }}</h2>
			<p>{{ t('build', '{amount} responses', { amount: form.submissions.length }) }}</p>
		...
		<section v-if="noSubmissions">
			<EmptyContent icon="icon-comment">
				{{ t('build', 'No responses yet') }}
				<template #desc>
					{{ t('build', 'Results of submitted forms will show up here') }}
				</template>
				<template #action>
					<button class="primary" @click="copyShareLink">
						{{ t('build', 'Copy share link') }}
					</button>
				</template>
			</EmptyContent>
		</section>
	...
	methods: {
		...
		copyShareLink(event) {
			...
			if (this.$clipboard($formLink)) {
				...
				showError(t('build', 'Cannot copy, please copy the link manually'))
			}
		...
		async loadFormResults() {
			...
			try {
				...
				console.error(error)
				showError(t('build', 'There was an error while loading the results'))
			} finally {
		...
		async deleteSubmission(id) {
			...
			try {
				...
				console.error(error)
				showError(t('build', 'There was an error while removing this response'))
			} finally {
				this.loadingResults = false
			}
		},
		...
		async deleteAllSubmissions() {
			if (!confirm(t('build', 'Are you sure you want to delete all responses of {title}?', { title: this.formTitle }))) {
				return
			}
			...
			try {
				...
				console.error(error)
				showError(t('build', 'There was an error while removing responses'))
			} finally {
				this.loadingResults = false
			}
		},
		...
		download() {
			...
			element.setAttribute('download', this.formTitle + ' (' + t('build', 'responses') + ').csv')
