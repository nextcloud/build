			...
			<li>
				...
				<label for="registered">
					<span class="icon-group">
						{{ t('build', 'Show to all users of this instance') }}
					</span>
				</label>
			</li>
			<li>
				...
				<label for="selected">
					<span class="icon-shared">
						{{ t('build', 'Choose users to share with') }}
					</span>
				</label>
		...
		<ul>
			...
			<li>
				...
				<label for="submitOnce">
					{{ t('build', 'Allow multiple responses per person') }}
				</label>
			</li>
			<li>
				<input id="expires"
				...
				<DatetimePicker v-show="formExpires"
					...
					:format="format"
					:minute-step="5"
					:placeholder="t('build', 'Expiration date')"
					:show-second="false"
...
export default {
	...
	data() {
		return {
			...
			lang: {
				...
				placeholder: {
					date: t('build', 'Select expiration date'),
				},
			},
			locale: 'en',
			format: {
	...
	methods: {
		...
		stringify(datetime) {
			...
			if (this.isExpired) {
				return t('build', 'Expired on {date}', { date })
			}
			return t('build', 'Expires on {date}', { date })
		},
