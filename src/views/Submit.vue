		...
		<AppContent>
			...
			<form v-if="!loading && !success"
				...
				<input ref="submitButton"
					class="primary"
					type="submit"
					:value="t('build', 'Submit')"
					:disabled="loading"
					:aria-label="t('build', 'Submit form')">
			</form>

			<EmptyContent v-else-if="loading" icon="icon-loading">
				{{ t('build', 'Submitting form …') }}
			</EmptyContent>

			<EmptyContent v-else-if="success" icon="icon-checkmark">
				{{ t('build', 'Thank you for completing the form!') }}
			</EmptyContent>
		</AppContent>
	</Content>
</template>
	...
	methods: {
		...
		async onSubmit() {
			...
			try {
				...
				console.error(error)
				showError(t('build', 'There was an error submitting the form'))
			} finally {
				this.loading = false
			}
		},
